...
class LamodaScraper:
    ...
    def create_csv(self, new_links, output_csv, category_key, grpc_client):
        ...
        with open(output_csv, 'a' if file_exists else 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for url in new_links:
                ...
                for image_url in image_urls:
                    ...
                    temp_tags.extend([image_url, image_url.split('/')[6].split('_')[0], category, embedding_norm, 'Lamoda'])
                    for tag in selected_tags:
    ...
    def create_and_append_csv(self, new_links, output_csv, category_key, grpc_client):
        """
    ...
    def update_links_file_json(self, filename, parsed_links):
        """
        Обновляет JSON-файл ссылок, добавляя новые ссылки с их статусом обработки.

        :param filename: Имя JSON-файла с уже существующими ссылками.
        :param parsed_links: Список новых ссылок, которые нужно проверить и добавить.
        :return: Список новых добавленных ссылок.
        """
        # Загружаем существующие ссылки из JSON-файла, если файл существует
        current_links = {}
        if os.path.exists(filename):
            with open(filename, "r", encoding="utf-8") as file:
                current_links = json.load(file)

        # Преобразуем существующие ссылки в множество для быстрого поиска
        current_urls = {entry['url'] for entry in current_links.get("links", [])}

        # Определяем новые ссылки, которых еще нет в JSON-файле
        new_links = [link for link in parsed_links if link not in current_urls]

        # Добавляем новые ссылки с полем "processed: False"
        for link in new_links:
            current_links.setdefault("links", []).append({"url": link, "processed": False})

        # Сохраняем обновлённый JSON в файл
        with open(filename, "w", encoding="utf-8") as file:
            json.dump(current_links, file, ensure_ascii=False, indent=4)

        print(f"{len(new_links)} новых ссылок добавлено.")
        return new_links  # Возвращаем список новых добавленных ссылок

    def create_and_append_csv_json(self, json_file, output_csv, category_key, grpc_client):
        """
        ...
        with open(output_csv, 'a', newline='', encoding='utf-8') as old_csvfile, \
            ...
            for link_entry in unprocessed_links:
                ...
                for image_url in image_urls:
                    ...
                    # Добавляем теги для выбранной категории
                    for tag in selected_tags:
                        row_data[tag] = attributes.get(tag, '')
