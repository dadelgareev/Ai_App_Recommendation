...
class LamodaScraper:
    ...
    def append_to_dataset(self, links, dataset_filename, images_dir, category):
        ...
        tags = self.tags.get(category, [])
        fieldnames = ['image_path'] + tags + ['Категория']  # Добавляем 'Категория' в заголовки CSV
        ...
        with open(dataset_filename, "a", newline='', encoding='utf-8') as csvfile:
            ...
            for url in links:
                ...
                # Извлекаем атрибуты по заданным тегам
                attributes_row = {tag: attributes.get(tag, 'Не указано') for tag in tags}
                ...
                for i, image_url in enumerate(image_urls):
                    ...
                    if image_path:
                        row = {'image_path': image_path, 'Категория': category_name, **attributes_row}
                        writer.writerow(row)