...
class LamodaScraper:
    ...
    def create_and_append_csv_json(self, json_file, output_csv, category_key):
        """
        ...
        with open(output_csv, 'a', newline='', encoding='utf-8') as old_csvfile, \
            ...
            for link_entry in unprocessed_links:
                ...
                for image_url in image_urls:
                    ...
                    # Получаем вектор эмбеддинга и обрабатываем возможные исключения
                    try:
                        embedding = self.model.get_embedding(image_path)
                        embedding_str = ','.join(map(str, embedding.cpu().numpy())) if embedding is not None else 'Ошибка'
                    except Exception as e:
                        print(f"Ошибка при создании эмбеддинга для {image_path}: {e}")
